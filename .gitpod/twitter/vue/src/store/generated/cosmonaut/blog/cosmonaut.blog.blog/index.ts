...
import { Follow } from "./module/types/blog/follow"
import { Params } from "./module/types/blog/params"
import { Post } from "./module/types/blog/post"
import { PostWithComments } from "./module/types/blog/query"


export { Comment, Follow, Params, Post, PostWithComments };
...
const getDefaultState = () => {
	return {
				...
				_Structure: {
						...
						Follow: getStructure(Follow.fromPartial({})),
						Params: getStructure(Params.fromPartial({})),
						Post: getStructure(Post.fromPartial({})),
						PostWithComments: getStructure(PostWithComments.fromPartial({})),
...
export default {
	...
	getters: {
			...
			return state.Comments[JSON.stringify(params)] ?? {}
				...
				getFollow: (state) => (params = { params: {}}) => {
					if (!(<any> params).query) {
						(<any> params).query=null
					}
			return state.Follow[JSON.stringify(params)] ?? {}
		},
				getFollowAll: (state) => (params = { params: {}}) => {
					if (!(<any> params).query) {
						(<any> params).query=null
					}
			return state.FollowAll[JSON.stringify(params)] ?? {}
		},
				getFollowingPosts: (state) => (params = { params: {}}) => {
					if (!(<any> params).query) {
						(<any> params).query=null
					}
			return state.FollowingPosts[JSON.stringify(params)] ?? {}
		},
				getFollowedBy: (state) => (params = { params: {}}) => {
					if (!(<any> params).query) {
						(<any> params).query=null
					}
			return state.FollowedBy[JSON.stringify(params)] ?? {}
		},
				getFollowedPosts: (state) => (params = { params: {}}) => {
					if (!(<any> params).query) {
						(<any> params).query=null
					}
			return state.FollowedPosts[JSON.stringify(params)] ?? {}
		},
	...
	actions: {
		...
		async QueryFollow({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryFollow( key.id)).data
				
					
				commit('QUERY', { query: 'Follow', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryFollow', payload: { options: { all }, params: {...key},query }})
				return getters['getFollow']( { params: {...key}, query}) ?? {}
			} catch (e) {
				throw new Error('QueryClient:QueryFollow API Node Unavailable. Could not perform query: ' + e.message)
		...
		async QueryFollowAll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryFollowAll(query)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.queryFollowAll({...query, 'pagination.key':(<any> value).pagination.next_key})).data
					value = mergeResults(value, next_values);
				}
				commit('QUERY', { query: 'FollowAll', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryFollowAll', payload: { options: { all }, params: {...key},query }})
				return getters['getFollowAll']( { params: {...key}, query}) ?? {}
			} catch (e) {
				throw new Error('QueryClient:QueryFollowAll API Node Unavailable. Could not perform query: ' + e.message)
		...
		async QueryFollowingPosts({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryFollowingPosts( key.id, query)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.queryFollowingPosts( key.id, {...query, 'pagination.key':(<any> value).pagination.next_key})).data
					value = mergeResults(value, next_values);
				}
				commit('QUERY', { query: 'FollowingPosts', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryFollowingPosts', payload: { options: { all }, params: {...key},query }})
				return getters['getFollowingPosts']( { params: {...key}, query}) ?? {}
			} catch (e) {
				throw new Error('QueryClient:QueryFollowingPosts API Node Unavailable. Could not perform query: ' + e.message)
		...
		async QueryFollowedBy({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryFollowedBy( key.creator, query)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.queryFollowedBy( key.creator, {...query, 'pagination.key':(<any> value).pagination.next_key})).data
					value = mergeResults(value, next_values);
				}
				commit('QUERY', { query: 'FollowedBy', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryFollowedBy', payload: { options: { all }, params: {...key},query }})
				return getters['getFollowedBy']( { params: {...key}, query}) ?? {}
			} catch (e) {
				throw new Error('QueryClient:QueryFollowedBy API Node Unavailable. Could not perform query: ' + e.message)
		...
		async QueryFollowedPosts({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryFollowedPosts( key.creator, query)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.queryFollowedPosts( key.creator, {...query, 'pagination.key':(<any> value).pagination.next_key})).data
					value = mergeResults(value, next_values);
				}
				commit('QUERY', { query: 'FollowedPosts', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryFollowedPosts', payload: { options: { all }, params: {...key},query }})
				return getters['getFollowedPosts']( { params: {...key}, query}) ?? {}
			} catch (e) {
				throw new Error('QueryClient:QueryFollowedPosts API Node Unavailable. Could not perform query: ' + e.message)
	...
	gas: "200000" }, memo})
		...
		async sendMsgCreateFollow({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgCreateFollow(value)
				const result = await txClient.signAndBroadcast([msg], {fee: { amount: fee, 
	gas: "200000" }, memo})
				return result
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgCreateFollow:Init Could not initialize signing client. Wallet is required.')
				}else{
					throw new Error('TxClient:MsgCreateFollow:Send Could not broadcast Tx: '+ e.message)
				}
			}
		},
	...
	gas: "200000" }, memo})
		...
		async MsgCreateFollow({ rootGetters }, { value }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgCreateFollow(value)
				return msg
				...
				if (e == MissingWalletError) {
					...
					throw new Error('TxClient:MsgCreateFollow:Create Could not create message: ' + e.message)
				}
			}
		},
